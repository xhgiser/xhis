...
function initCesium() {
  ...
  viewer.value.camera.flyTo({
    ...
    destination: Cesium.Cartesian3.fromDegrees(118.649, 37.4, 1000),
    orientation: {
      // 指向 旋转角 正东为90°
      heading: 0,
      // 俯仰角 水平为0°
...
const dataOption = [
  {
    ...
    children: [
      ...
      {
        value: 'DongYing_XinHuiDongHaiAn',
        label: '新汇东海岸',
      },
      {
        value: 'DongYing_WuGanQu',
        label: '五干渠',
      },
    ],
  },
...
function flyToData(value) {
  viewer.value.scene.imageryLayers.addImageryProvider(
    new Cesium.UrlTemplateImageryProvider({
      url: 'http://192.168.188.8/Image/' + value[1] + '/{z}/{x}/{y}.png',
    })
  )
  viewer.value.camera.flyTo({
    // Cesium的坐标是以地心为原点，一向指向南美洲，一向指向亚洲，一向指向北极州
