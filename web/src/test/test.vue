...
let geoserverLayersArray = []
function addGeoserverLayer(layers) {
  layers.forEach((layerObject) => {
    geoserverLayersArray.push(layerObject.layer)
  })
}
addGeoserverLayer(waterLayers)
layers = layers.concat(geoserverLayersArray)
console.log(111)
/* geoserver图层添加至地图 */
// let geoserverLayersArray = []
// const geoserverLayres = MapConfig.waterLayers
// // function addLayersToMap() {
// geoserverLayres.forEach((layerObject, index) => {
//   if (layerObject) {
//     layers.push(layerObject.layer)
//     //geoserverLayersArray.push(layerObject.layer)
//     //layers.unshift(layerObject.layer)
